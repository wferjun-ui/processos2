    ...
    public class DatabaseService
    {
            ...
            using (var conn = GetConnection())
            {
                ...
                using (var transaction = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        // 2. RESET FORÇADO DO ADMIN
                        // Garante que o usuário admin exista com a senha padrão 'admin'
        ...
        public (bool Success, bool IsAdmin, string Username) Login(string username, string password)
        {
            using (var conn = GetConnection())
            {
                ...
                {
                    // Converte long/int do SQLite para boolean
            ...
            using (var conn = GetConnection())
            {
                ...
                using (var trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        // Correção na parametrização para evitar erro de parser
                        var parametrosProcesso = new 
                        { 
                            id = procId, 
                            num = numero, 
                            pac = paciente, 
                            juiz = juiz, 
                            classificacao = classificacao, // Nome explícito
                            dt = dtStr, 
                            prazo = prazoStr 
                        };
