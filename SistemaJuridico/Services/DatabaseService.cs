    ...
    public class DatabaseService
    {
        ...
        public void Initialize()
        {
            ...
            try
            {
                ...
                // Reset Admin (Se necessário, descomente para resetar a senha)
                var adminExists = conn.ExecuteScalar<int>("SELECT count(*) FROM usuarios WHERE lower(username) = 'admin'", transaction: transaction);
                if (adminExists == 0)
                {
                    var salt = GenerateSalt();
                    var hash = HashPassword("admin", salt);
                    conn.Execute(@"INSERT INTO usuarios (id, username, password_hash, salt, is_admin, email) 
                                   VALUES (@id, 'admin', @h, @s, 1, 'admin@sistema.local')",
                                   new { id = Guid.NewGuid().ToString(), h = hash, s = salt }, transaction: transaction);
                }
        ...
        {
            using var conn = GetConnection();
            // Mapeia 1 (int) para true (bool)
            var users = conn.Query("SELECT id, username, email, is_admin FROM usuarios");
        ...
        public void SalvarNovoProcesso(string numero, string paciente, string juiz, string reu, string classificacao, string usuarioLogado)
        {
            ...
            try
            {
                ...
                var dtStr = hoje.ToString("dd/MM/yyyy");
                ...
                conn.Execute(@"INSERT INTO processos (id, numero, paciente, juiz, classificacao, status_fase, ultima_atualizacao, cache_proximo_prazo)
                    ...
                    new { id = procId, num = numero, pac = paciente, juiz, classificacao, dt = dtStr, prazo = prazoStr }, trans);
